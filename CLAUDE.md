...
每条设计内容必须标注三级来源，输出到 HTML 时一律使用纯文本方括号，不使用 emoji：
- **[知识库]** — 精确命中文档（标注文档+章节）
- **[跨域参考]** — 跨域相似页面截图/模式
...
10. **黄金样例只约束输出形态** — `examples/hjb-refund-golden-pattern.md` 记录 P2/P3/P4 结构，不参与知识路由
11. **模板占位提示不使用斜体** — `.placeholder` 保持 normal，避免待填充文本显得像注释或弱提示
12. **P1 首屏不得输出裸段落** — Slot A/B 使用 `.content-item + .source-note`，Slot C 使用 `.info-table`，来源不得混在正文末尾
...
业务知识库/
  ...
  examples/             - 验证案例与输出形态样例（物理隔离，不被 AI 误命中）
```
