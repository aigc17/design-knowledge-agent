...
P4 阶段：填充 Slot H（四维自检）+ 定稿 Slot I-summary（证据汇总）。
...
- **业务规则一致性**：每条规则标注 📗 来源
- **UI 规范合规**：尺寸、组件、视觉一致性
- **信息不足项**：KB 缺失的信息，需人工补充

定位 `id="slot-h"`，替换 placeholder。

### 4. 定稿 I-summary

定位 `id="slot-i-summary"`，替换 placeholder，写入：
...
**注意**：只操作 `id="slot-i-summary"`，不触碰 `id="slot-i-gallery"`。
...
输出最终 report HTML 路径，告知用户全部 9+1 槽位已填满。
