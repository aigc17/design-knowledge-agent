...
**★ KB 覆盖度预检**（P1 末尾新增）：
填完 A-D 后，扫描需求涉及的关键决策点，对照 KB 命中情况：
...
**★ 检查点**：
> Part 1（需求认知）+ KB 覆盖度预检已完成。理解是否准确？确认后进入 P2。
...
- 主链路流程节点（标注新增/复用）
- 分支/异常路径
- **流程同构分析**：将需求流程与 KB 中同原型流程并排对比

  ```
  交易购买（原型）: 商品选择 → 交易确认 → 支付鉴权 → 购买结果
  退款流程（本次）: 订单记录 → 退款确认 → [无鉴权] → 退款结果
  映射：确认≈确认、结果≈结果、鉴权环节省略（退钱不需密码）
  ```

**填充 Slot F**：
- 页面清单表格：名称 | 类型(new/reuse) | 参考来源(📗/📘/🔮) | 状态
...
- 📘 跨域参考汇总表：文档 → 页面 → 截图 → 设计页 → adopted? → 参考了什么
- 🔮 纯推理清单：Slot → 内容 → 推理原因
- KB 建议：KB 应补充的内容（未覆盖的决策点）
- ⚠️ 矛盾检测：如发现 KB 内不同文档间的数据矛盾，标红提醒
